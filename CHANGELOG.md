...
### 变更
- 摘要格式修正：移除 `\vspace*{\fill}`，摘要自然置于第一页底部
- 模板代码清理：三种模板（F002-1/2/3/4）的 abstract 环境统一去除 vfill
- Agent 职责描述完善：内容优化 Agent 输入契约说明更清晰
- 六 Agent 流水线图示更新：明确标注 Doubao-seed-2.0-pro 位置
- 演示文档同步：demo .tex 文件同步更新摘要格式

...
同步本地 README 到 GitHub（Step 3）

## v3.3.0 — 2026-05-02

v3.3.0新增文档质量评分系统（多维度自动评估+改进建议）
...
v3.2.0新增协作式写作模式（多角色协作写作+自动风格统一+冲突解决）

## v3.2.0 — 2026-05-02

v3.1.0新增协作式写作模式（多角色协作写作+自动风格统一+冲突解决）

## v3.1.0 — 2026-05-02

Round 5更新：文档写作能力全面升级

## v3.0.0 — 2026-05-02

v2.0.0重大升级：协作式写作工作流（多人协作+文档状态机+实时冲突解决+版本控制）

## v2.0.0 — 2026-05-02

### 重大升级：协作式写作工作流

**核心能力**：多人协作写文档时，自动协调各方输入，管理文档版本，解决冲突，保持一致性。

### 新增功能

- **协作架构**：主编/作者/审核/读者四角色体系，支持权限分级
- **文档状态机**：草稿 → 审核中 → 修改中 → 已批准 → 已发布 → 已归档，完整生命周期管理
- **协作消息总线**：实时广播章节锁定、内容更新、评论、审核请求、冲突检测等事件
- **冲突解决器**：支持 merge/author_choice/latest_wins/manual 四种策略自动处理版本冲突
- **版本控制**：commit/diff/rollback/branch 完整版本管理能力
- **实时协作协议**：WebSocket + Operational Transform，1秒同步间隔

### 适用场景

- 多部门协作撰写项目申报书（技术+产品+法务）
- 学术论文多作者协同写作
- 商业文档多方审核修订流程
- 大型文档分工编写与版本合并
...
```
协作架构：用户A/B/C → 协作总线 → 文档状态机 → 文档服务器
冲突检测：Operational Transform + 自动合并算法
实时同步：WebSocket + 1秒心跳
```

## v1.10.0 — 2026-05-02

v1.9.0新增多格式联动写作（一套源文档自动生成Markdown/LaTeX/HTML/Word/PPT）

## v1.9.0 — 2026-05-02

v1.9 新增多格式联动写作（一个核心内容，多种格式输出，无需重复写。写一次，自动生成 Markdown/LaTeX/HTML/Word/PPT）

## v1.8.0 — 2026-05-02

v1.7.0新增跨文档知识图谱（术语/配置一致性检查+自动同步更新）

## v1.7.0 — 2026-05-02

v1.7 新增跨文档知识图谱（自动维护多文档间的知识一致性，确保同一概念在不同文档中表述统一）

## v1.6.0 — 2026-05-02

## v1.5.0 — 2026-05-02

v1.2.0新增文档质量评分（多维度质量评分系统）+ LaTeX公式增强（公式识别+自动纠错）
...
### 新增
- **文档质量评分**：文档生成后自动多维度评分（可读性/完整性/引用准确性/结构/专业性），输出具体改进建议
- **评分维度体系**：可读性(20%)/完整性(25%)/引用准确性(20%)/结构合理性(20%)/专业性(15%)五维度加权评分
- **评分报告格式**：综合评分、各维度得分、等级、问题摘要、改进建议（按优先级排序）
- **评分阈值标准**：≥90优秀/75-89良好/60-74及格/<60不及格

### 变更
- SKILL.md frontmatter：version "v1.1.0" → "v1.2.0"
- SKILL.md：新增"文档质量评分（v1.2 新增）"章节

## v1.1.0 — 2026-05-02

### 新增
- **联网写作**：用户只给主题，AI自动联网搜索最新文献和数据，自动融入文档，解决素材痛点
- **联网写作模块**：完整搜索策略/执行流程/搜索报告格式/数据可信度分级
- **数据支撑标记**：{{NEED_DATA:章节名}}，规划 Agent 自动检测并触发搜索
- **搜索报告格式**：包含搜索关键词/搜索结果表格/数据提取/自动生成参考文献
- **搜索策略分类**：学术论文/项目申报书/查新报告/商业计划书各有不同数据来源
- **数据可信度分级**：A级（核心期刊/政府数据）/ B级（普通期刊/行业报告）/ C级（博客/论坛）
- **搜索触发场景**：用户说"帮我搜"或规划 Agent 发现章节缺少数据支撑时自动触发

### 变更
- SKILL.md frontmatter：version "v1.0.0" → "v1.1.0"
- SKILL.md description：新增触发场景"用户只给主题没有素材，要求自动搜索文献"
- SKILL.md description：核心能力新增"联网写作（v1.1 新增）"
- SKILL.md：新增"联网写作模块（v1.1 新增）"章节
